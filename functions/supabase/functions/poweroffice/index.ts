...
async function findOrCreateCustomer(customer: any) {
  ...
  const fullName = String(customer.name || "Ukjent kunde").trim().replace(/\s+/g, " ");
  const nameParts = fullName.split(" ").filter(Boolean);
  const firstName = String(customer.firstName || (nameParts.length > 1 ? nameParts.slice(0, -1).join(" ") : nameParts[0] || "Ukjent")).trim();
  const lastName = String(customer.lastName || (nameParts.length > 1 ? nameParts[nameParts.length - 1] : "Kunde")).trim();
  const payload = compact({
    Name: fullName || `${firstName} ${lastName}`,
    FirstName: firstName || "Ukjent",
...
async function createInvoiceDraft(body: any) {
  ...
  const job = body.job || {};
  const salesOrdersPath = env("POWEROFFICE_SALES_ORDERS_PATH", "/SalesOrders");
  const externalImportReference = `rettilomma-jobb-${job.id}`;
  ...
  const line = compact({
    ...
    Quantity: quantity,
    UnitPrice: unitPrice,
    ExternalLineReference: `rettilomma-jobb-linje-${job.id}`,
  ...
  const payload = compact({
    ...
    OrderDate: invoice.date || job.date,
