...
function firmaFromName(name:string){ const m=name.match(/handverker-backup-firma-(.+?)-(\d{14})\.json$/); return m?m[1]:null; }
Deno.serve(async req=>{
 ...
 const body=await req.json().catch(()=>({})); let requested=String(body?.firma_id||"");
 if(!isSys){ requested=String(a?.firma_id||""); if(!requested) return out({error:"Fant ikke firma for innlogget administrator."},403); }
 const rows:any[]=[];
 for(const kind of ["auto","manual","pre-restore"]){
   const {data:days,error}=await service.storage.from("handverker-backups").list(kind,{limit:100,sortBy:{column:"name",order:"desc"}});
   if(error&&kind==="auto") return out({error:error.message},500);
   for(const day of days||[]){
     if(!day.name) continue;
     const {data:dayFiles}=await service.storage.from("handverker-backups").list(`${kind}/${day.name}`,{limit:200,sortBy:{column:"name",order:"desc"}});
     for(const f of dayFiles||[]){
       const fid=firmaFromName(f.name)||null;
       // Full backup kan brukes for ethvert firma. Firma-backup vises bare for valgt firma.
       if(requested&&fid&&fid!==requested) continue;
       rows.push({type:kind,path:`${kind}/${day.name}/${f.name}`,name:f.name,firma_id:fid,created_at:f.created_at||f.updated_at||day.name,size:f.metadata?.size||null});
     }
   }
 }
 rows.sort((a,b)=>String(b.created_at).localeCompare(String(a.created_at)));
 return out({ok:true,is_sysadmin:isSys,firma_id:a?.firma_id||null,backups:rows.slice(0,300)});
});
